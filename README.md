...
- 本仓库与提交组委会审查的代码包相比，**算法零改动**，仅按官方《开源代码规范》做了规范性调整：目录重排（`src/`、`outputs/`）、去除写死的本机路径（改为必填参数）、运行配置与命令落盘、缺路径时的明确报错、新增 `configs/` 记录与 `LICENSE`/`NOTICE`、两个类名改为 PascalCase、关键函数补充中文文档字符串。以上改动均已用仓库内权重重新推理验证：输出与线上提交文件逐位一致。
